...
import rclpy
from controller_manager_msgs.srv import ListControllers, ListControllerTypes, ReloadControllerLibraries, LoadController, SwitchController, UnloadController
from ros2node.api import NodeNameCompleter
from ros2param.api import ParameterNameCompleter, call_list_parameters
...
def service_caller(service_name, service_type, request):
    try:
        ...
        node = rclpy.create_node('ros2controlcli_{}_requester'.format(service_name.replace("/","")))

        cli = node.create_client(service_type, service_name)

        if not cli.service_is_ready():
            node.get_logger().debug('waiting for service {} to become available...'.format(service_name))
...
def list_controllers(controller_manager_name):
    ...
    return service_caller('{}/list_controllers'.format(controller_manager_name), ListControllers, request)

def list_controller_types(controller_manager_name):
    request = ListControllerTypes.Request()
    return service_caller('{}/list_controller_types'.format(controller_manager_name), ListControllerTypes, request)

def reload_controller_libraries(controller_manager_name, force_kill):
    request = ReloadControllerLibraries.Request()
    request.force_kill = force_kill
    return service_caller('{}/reload_controller_libraries'.format(controller_manager_name), ReloadControllerLibraries, request)

def load_controller(controller_manager_name, controller_name):
    request = LoadController.Request()
    request.name = controller_name
    return service_caller('{}/load_controller'.format(controller_manager_name), LoadController, request)

def switch_controllers(controller_manager_name, stop_controllers, start_controllers, strict, start_asap, timeout):
    ...
    return service_caller('{}/switch_controller'.format(controller_manager_name), SwitchController, request)

def unload_controller(controller_manager_name, controller_name):
    request = UnloadController.Request()
    request.name = controller_name
    return service_caller('{}/unload_controller'.format(controller_manager_name), UnloadController, request)
...
def add_controller_mgr_parsers(parser):
    """Add parser arguments to get a node name for the controller manager.
       defaults to controller_manager
     """
