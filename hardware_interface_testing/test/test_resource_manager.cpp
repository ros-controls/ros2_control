...
TEST_F(ResourceManagerTest, managing_controllers_reference_interfaces)
{
  ...
  // claim interfaces in a scope that deletes them after
  {
    ...
    EXPECT_EQ(claimed_itf1.get_value<double>().value(), 1.0);
    EXPECT_EQ(claimed_itf3.get_value<double>().value(), 3.0);

    claimed_itf1.set_value(11.1);
    claimed_itf3.set_value(33.3);
    EXPECT_EQ(claimed_itf1.get_value<double>().value(), 11.1);
    EXPECT_EQ(claimed_itf3.get_value<double>().value(), 33.3);
...
public:
  ...
  {
    double prev_act_state_value = state_itfs[0].get_value<double>().value();
    double prev_system_state_value = state_itfs[1].get_value<double>().value();
    ...
    for (size_t i = 1; i < 100; i++)
    {
      ...
      if (i % (cm_update_rate_ / system_rw_rate_) == 0 && test_for_changing_values)
      {
        ...
        prev_system_state_value = claimed_itfs[1].get_value<double>().value() / 2.0;
        claimed_itfs[1].set_value(claimed_itfs[1].get_value<double>().value() + 20.0);
      }
      if (i % (cm_update_rate_ / actuator_rw_rate_) == 0 && test_for_changing_values)
      {
        // The values are computations exactly within the test_components
        prev_act_state_value = claimed_itfs[0].get_value<double>().value() / 2.0;
        claimed_itfs[0].set_value(claimed_itfs[0].get_value<double>().value() + 10.0);
      }
      ...
      ASSERT_EQ(state_itfs[0].get_value<double>().value(), prev_act_state_value);
      ASSERT_EQ(state_itfs[1].get_value<double>().value(), prev_system_state_value);
...
public:
  ...
  void check_read_and_write_cycles(bool check_for_updated_values)
  {
    ...
    for (size_t i = 1; i < 100; i++)
    {
      ...
      claimed_itfs[0].set_value(claimed_itfs[0].get_value<double>().value() + actuator_increment);
      claimed_itfs[1].set_value(claimed_itfs[1].get_value<double>().value() + system_increment);
      ...
      ASSERT_NEAR(
        state_itfs[0].get_value<double>().value(), prev_act_state_value, actuator_increment / 2.0);
      ASSERT_NEAR(
        state_itfs[1].get_value<double>().value(), prev_system_state_value, system_increment / 2.0);
      auto [ok_write, failed_hardware_names_write] = rm->write(time, duration);
      std::this_thread::sleep_for(std::chrono::milliseconds(1));
      EXPECT_TRUE(ok_write);
      EXPECT_TRUE(failed_hardware_names_write.empty());
