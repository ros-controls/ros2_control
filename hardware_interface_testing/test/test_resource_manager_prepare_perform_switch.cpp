  ...
  when_system_active_and_actuator_unconfigured_expect_system_passing)
{
  ...
  EXPECT_EQ(claimed_system_acceleration_state_->get_value<double>().value(), 1.0);
  EXPECT_EQ(claimed_actuator_position_state_->get_value<double>().value(), 0.0);
  EXPECT_TRUE(rm_->perform_command_mode_switch(legal_keys_system, legal_keys_system));
  EXPECT_EQ(claimed_system_acceleration_state_->get_value<double>().value(), 101.0);
  EXPECT_EQ(claimed_actuator_position_state_->get_value<double>().value(), 0.0);

  EXPECT_TRUE(rm_->prepare_command_mode_switch(legal_keys_system, empty_keys));
  EXPECT_EQ(claimed_system_acceleration_state_->get_value<double>().value(), 102.0);
  EXPECT_EQ(claimed_actuator_position_state_->get_value<double>().value(), 0.0);
  EXPECT_TRUE(rm_->perform_command_mode_switch(legal_keys_system, empty_keys));
  EXPECT_EQ(claimed_system_acceleration_state_->get_value<double>().value(), 202.0);
  EXPECT_EQ(claimed_actuator_position_state_->get_value<double>().value(), 0.0);

  EXPECT_TRUE(rm_->prepare_command_mode_switch(empty_keys, legal_keys_system));
  EXPECT_EQ(claimed_system_acceleration_state_->get_value<double>().value(), 203.0);
  EXPECT_EQ(claimed_actuator_position_state_->get_value<double>().value(), 0.0);
  EXPECT_FALSE(rm_->perform_command_mode_switch(empty_keys, legal_keys_system));
  EXPECT_EQ(claimed_system_acceleration_state_->get_value<double>().value(), 303.0);
  EXPECT_EQ(claimed_actuator_position_state_->get_value<double>().value(), 0.0);
  ...
  EXPECT_EQ(claimed_system_acceleration_state_->get_value<double>().value(), 403.0);
  EXPECT_EQ(claimed_actuator_position_state_->get_value<double>().value(), 0.0);
  ...
  EXPECT_EQ(claimed_system_acceleration_state_->get_value<double>().value(), 503.0);
  EXPECT_EQ(claimed_actuator_position_state_->get_value<double>().value(), 0.0);
  ...
  EXPECT_EQ(claimed_system_acceleration_state_->get_value<double>().value(), 603.0);
  EXPECT_EQ(claimed_actuator_position_state_->get_value<double>().value(), 0.0);
};
  ...
  ResourceManagerPreparePerformTest, when_system_active_and_actuator_inactive_expect_both_passing)
{
  ...
  EXPECT_TRUE(rm_->prepare_command_mode_switch(legal_keys_system, legal_keys_system));
  EXPECT_EQ(claimed_system_acceleration_state_->get_value<double>().value(), 1.0);
  EXPECT_EQ(claimed_actuator_position_state_->get_value<double>().value(), 1.0);
  EXPECT_TRUE(rm_->perform_command_mode_switch(legal_keys_system, legal_keys_system));
  EXPECT_EQ(claimed_system_acceleration_state_->get_value<double>().value(), 101.0);
  EXPECT_EQ(claimed_actuator_position_state_->get_value<double>().value(), 101.0);

  EXPECT_TRUE(rm_->prepare_command_mode_switch(legal_keys_system, empty_keys));
  EXPECT_EQ(claimed_system_acceleration_state_->get_value<double>().value(), 102.0);
  EXPECT_EQ(claimed_actuator_position_state_->get_value<double>().value(), 102.0);
  EXPECT_TRUE(rm_->perform_command_mode_switch(legal_keys_system, empty_keys));
  EXPECT_EQ(claimed_system_acceleration_state_->get_value<double>().value(), 202.0);
  EXPECT_EQ(claimed_actuator_position_state_->get_value<double>().value(), 202.0);

  EXPECT_TRUE(rm_->prepare_command_mode_switch(empty_keys, legal_keys_system));
  EXPECT_EQ(claimed_system_acceleration_state_->get_value<double>().value(), 203.0);
  EXPECT_EQ(claimed_actuator_position_state_->get_value<double>().value(), 203.0);
  EXPECT_FALSE(rm_->perform_command_mode_switch(empty_keys, legal_keys_system));
  EXPECT_EQ(claimed_system_acceleration_state_->get_value<double>().value(), 303.0);
  EXPECT_EQ(claimed_actuator_position_state_->get_value<double>().value(), 303.0);
  ...
  EXPECT_EQ(claimed_system_acceleration_state_->get_value<double>().value(), 304.0);
  EXPECT_EQ(claimed_actuator_position_state_->get_value<double>().value(), 304.0);
  EXPECT_TRUE(rm_->perform_command_mode_switch(legal_keys_actuator, legal_keys_actuator));
  EXPECT_EQ(claimed_system_acceleration_state_->get_value<double>().value(), 404.0);
  EXPECT_EQ(claimed_actuator_position_state_->get_value<double>().value(), 404.0);

  EXPECT_TRUE(rm_->prepare_command_mode_switch(legal_keys_actuator, empty_keys));
  EXPECT_EQ(claimed_system_acceleration_state_->get_value<double>().value(), 405.0);
  EXPECT_EQ(claimed_actuator_position_state_->get_value<double>().value(), 405.0);
  EXPECT_TRUE(rm_->perform_command_mode_switch(legal_keys_actuator, empty_keys));
  EXPECT_EQ(claimed_system_acceleration_state_->get_value<double>().value(), 505.0);
  EXPECT_EQ(claimed_actuator_position_state_->get_value<double>().value(), 505.0);

  EXPECT_TRUE(rm_->prepare_command_mode_switch(empty_keys, legal_keys_actuator));
  EXPECT_EQ(claimed_system_acceleration_state_->get_value<double>().value(), 506.0);
  EXPECT_EQ(claimed_actuator_position_state_->get_value<double>().value(), 506.0);
  EXPECT_FALSE(rm_->perform_command_mode_switch(empty_keys, legal_keys_actuator));
  EXPECT_EQ(claimed_system_acceleration_state_->get_value<double>().value(), 606.0);
  EXPECT_EQ(claimed_actuator_position_state_->get_value<double>().value(), 606.0);
};
  ...
  when_system_unconfigured_and_actuator_active_expect_actuator_passing)
{
  ...
  EXPECT_EQ(claimed_system_acceleration_state_->get_value<double>().value(), 0.0);
  EXPECT_EQ(claimed_actuator_position_state_->get_value<double>().value(), 0.0);
  EXPECT_TRUE(rm_->perform_command_mode_switch(legal_keys_system, legal_keys_system));
  EXPECT_EQ(claimed_system_acceleration_state_->get_value<double>().value(), 0.0);
  EXPECT_EQ(claimed_actuator_position_state_->get_value<double>().value(), 100.0);
  ...
  EXPECT_TRUE(rm_->perform_command_mode_switch(legal_keys_system, empty_keys));
  EXPECT_EQ(claimed_system_acceleration_state_->get_value<double>().value(), 0.0);
  EXPECT_EQ(claimed_actuator_position_state_->get_value<double>().value(), 200.0);
  ...
  EXPECT_EQ(claimed_system_acceleration_state_->get_value<double>().value(), 0.0);
  EXPECT_EQ(claimed_actuator_position_state_->get_value<double>().value(), 300.0);

  // When TestActuatorHardware is INACTIVE expect OK
  EXPECT_TRUE(rm_->prepare_command_mode_switch(legal_keys_actuator, legal_keys_actuator));
  EXPECT_EQ(claimed_system_acceleration_state_->get_value<double>().value(), 0.0);
  EXPECT_EQ(claimed_actuator_position_state_->get_value<double>().value(), 301.0);
  EXPECT_TRUE(rm_->perform_command_mode_switch(legal_keys_actuator, legal_keys_actuator));
  EXPECT_EQ(claimed_system_acceleration_state_->get_value<double>().value(), 0.0);
  EXPECT_EQ(claimed_actuator_position_state_->get_value<double>().value(), 401.0);

  EXPECT_TRUE(rm_->prepare_command_mode_switch(legal_keys_actuator, empty_keys));
  EXPECT_EQ(claimed_system_acceleration_state_->get_value<double>().value(), 0.0);
  EXPECT_EQ(claimed_actuator_position_state_->get_value<double>().value(), 402.0);
  EXPECT_TRUE(rm_->perform_command_mode_switch(legal_keys_actuator, empty_keys));
  EXPECT_EQ(claimed_system_acceleration_state_->get_value<double>().value(), 0.0);
  EXPECT_EQ(claimed_actuator_position_state_->get_value<double>().value(), 502.0);

  EXPECT_TRUE(rm_->prepare_command_mode_switch(empty_keys, legal_keys_actuator));
  EXPECT_EQ(claimed_system_acceleration_state_->get_value<double>().value(), 0.0);
  EXPECT_EQ(claimed_actuator_position_state_->get_value<double>().value(), 503.0);
  EXPECT_TRUE(rm_->perform_command_mode_switch(empty_keys, legal_keys_actuator));
  EXPECT_EQ(claimed_system_acceleration_state_->get_value<double>().value(), 0.0);
  EXPECT_EQ(claimed_actuator_position_state_->get_value<double>().value(), 603.0);
};
