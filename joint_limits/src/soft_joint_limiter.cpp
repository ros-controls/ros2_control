...
public:
    ...
    const rclcpp::Duration & dt)
  {
    ...
    if (hard_limits.has_velocity_limits)
    {
        ...
        std::isfinite(position))
      {
        ...
        if ((position < hard_limits.min_position) || (position > hard_limits.max_position))
        {
          ...
          (position < soft_joint_limits.min_position) ||
          (position > soft_joint_limits.max_position))
        {
    ...
    if (desired.has_velocity())
    {
      ...
      {
        soft_min_vel = std::max(
          actual.velocity.value() - hard_limits.max_acceleration * dt_seconds, soft_min_vel);
        soft_max_vel = std::min(
          actual.velocity.value() + hard_limits.max_acceleration * dt_seconds, soft_max_vel);
      }
