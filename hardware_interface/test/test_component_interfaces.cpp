...
TEST(TestComponentInterfaces, dummy_actuator)
{
  ...
  for (auto step = 0u; step < 10; ++step)
  {
    ...
    ASSERT_TRUE(std::isnan(state_interfaces[0]->get_value<double>().value()));  // position value
    ASSERT_TRUE(std::isnan(state_interfaces[1]->get_value<double>().value()));  // velocity
  ...
  for (auto step = 0u; step < 10; ++step)
  {
    ...
    EXPECT_EQ(
      step * velocity_value, state_interfaces[0]->get_value<double>().value());  // position value
    EXPECT_EQ(
      step ? velocity_value : 0, state_interfaces[1]->get_value<double>().value());  // velocity
  ...
  for (auto step = 0u; step < 10; ++step)
  {
    ...
    EXPECT_EQ(
      ...
      state_interfaces[0]->get_value<double>().value());                          // position value
    EXPECT_EQ(velocity_value, state_interfaces[1]->get_value<double>().value());  // velocity
  ...
  for (auto step = 0u; step < 10; ++step)
  {
    ...
    EXPECT_EQ(
      20 * velocity_value, state_interfaces[0]->get_value<double>().value());  // position value
    EXPECT_EQ(0, state_interfaces[1]->get_value<double>().value());            // velocity
...
TEST(TestComponentInterfaces, dummy_actuator_default)
{
  ...
  for (auto step = 0u; step < 10; ++step)
  {
    ...
    ASSERT_TRUE(
      std::isnan(state_interfaces[si_joint1_pos]->get_value<double>().value()));  // position value
    ASSERT_TRUE(
      std::isnan(state_interfaces[si_joint1_vel]->get_value<double>().value()));  // velocity
  ...
  for (auto step = 0u; step < 10; ++step)
  {
    ...
    EXPECT_EQ(
      step * velocity_value,
      state_interfaces[si_joint1_pos]->get_value<double>().value());  // position value
    EXPECT_EQ(
      step ? velocity_value : 0,
      state_interfaces[si_joint1_vel]->get_value<double>().value());  // velocity
  ...
  for (auto step = 0u; step < 10; ++step)
  {
    ...
    EXPECT_EQ(
      ...
      state_interfaces[si_joint1_pos]->get_value<double>().value());  // position value
    EXPECT_EQ(
      velocity_value, state_interfaces[si_joint1_vel]->get_value<double>().value());  // velocity
  ...
  for (auto step = 0u; step < 10; ++step)
  {
    ...
    EXPECT_EQ(
      20 * velocity_value,
      state_interfaces[si_joint1_pos]->get_value<double>().value());             // position value
    EXPECT_EQ(0, state_interfaces[si_joint1_vel]->get_value<double>().value());  // velocity
...
TEST(TestComponentInterfaces, dummy_sensor)
{
  ...
  EXPECT_TRUE(std::isnan(state_interfaces[0]->get_value<double>().value()));
  ...
  EXPECT_EQ(lifecycle_msgs::msg::State::PRIMARY_STATE_INACTIVE, state.id());
  EXPECT_EQ(hardware_interface::lifecycle_state_names::INACTIVE, state.label());
  EXPECT_EQ(0.0, state_interfaces[0]->get_value<double>().value());

  // It can read now
  sensor_hw.read(TIME, PERIOD);
  EXPECT_EQ(0x666, state_interfaces[0]->get_value<double>().value());
}
// END
...
TEST(TestComponentInterfaces, dummy_sensor_default)
{
  ...
  sensor_hw.read(TIME, PERIOD);
  EXPECT_TRUE(std::isnan(state_interfaces[si_sens1_vol]->get_value<double>().value()));

  // Updated because is is INACTIVE
  state = sensor_hw.configure();
  EXPECT_EQ(lifecycle_msgs::msg::State::PRIMARY_STATE_INACTIVE, state.id());
  EXPECT_EQ(hardware_interface::lifecycle_state_names::INACTIVE, state.label());
  EXPECT_EQ(0.0, state_interfaces[si_sens1_vol]->get_value<double>().value());

  // It can read now
  sensor_hw.read(TIME, PERIOD);
  EXPECT_EQ(0x666, state_interfaces[si_sens1_vol]->get_value<double>().value());
}
...
TEST(TestComponentInterfaces, dummy_sensor_default_joint)
{
  ...
  EXPECT_TRUE(std::isnan(state_interfaces[si_joint1_pos]->get_value<double>().value()));
  ...
  EXPECT_EQ(0.0, state_interfaces[si_sens1_vol]->get_value<double>().value());
  EXPECT_EQ(10.0, state_interfaces[si_joint1_pos]->get_value<double>().value());

  // It can read now
  sensor_hw.read(TIME, PERIOD);
  EXPECT_EQ(0x666, state_interfaces[si_sens1_vol]->get_value<double>().value());
  EXPECT_EQ(0x777, state_interfaces[si_joint1_pos]->get_value<double>().value());
}
...
TEST(TestComponentInterfaces, dummy_system)
{
  ...
  for (auto step = 0u; step < 10; ++step)
  {
    ...
    ASSERT_TRUE(std::isnan(state_interfaces[0]->get_value<double>().value()));  // position value
    ASSERT_TRUE(std::isnan(state_interfaces[1]->get_value<double>().value()));  // velocity
    ASSERT_TRUE(std::isnan(state_interfaces[2]->get_value<double>().value()));  // position value
    ASSERT_TRUE(std::isnan(state_interfaces[3]->get_value<double>().value()));  // velocity
    ASSERT_TRUE(std::isnan(state_interfaces[4]->get_value<double>().value()));  // position value
    ASSERT_TRUE(std::isnan(state_interfaces[5]->get_value<double>().value()));  // velocity
  ...
  for (auto step = 0u; step < 10; ++step)
  {
    ...
    EXPECT_EQ(
      step ? velocity_value : 0, state_interfaces[1]->get_value<double>().value());  // velocity
    EXPECT_EQ(
      step * velocity_value, state_interfaces[2]->get_value<double>().value());  // position value
    EXPECT_EQ(
      step ? velocity_value : 0, state_interfaces[3]->get_value<double>().value());  // velocity
    EXPECT_EQ(
      step * velocity_value, state_interfaces[4]->get_value<double>().value());  // position value
    EXPECT_EQ(
      step ? velocity_value : 0, state_interfaces[5]->get_value<double>().value());  // velocity
  ...
  for (auto step = 0u; step < 10; ++step)
  {
    ...
    EXPECT_EQ(
      (10 + step) * velocity_value,
      state_interfaces[2]->get_value<double>().value());                          // position value
    EXPECT_EQ(velocity_value, state_interfaces[3]->get_value<double>().value());  // velocity
    EXPECT_EQ(
      (10 + step) * velocity_value,
      state_interfaces[4]->get_value<double>().value());                          // position value
    EXPECT_EQ(velocity_value, state_interfaces[5]->get_value<double>().value());  // velocity
  ...
  for (auto step = 0u; step < 10; ++step)
  {
    ...
    EXPECT_EQ(
      20 * velocity_value, state_interfaces[0]->get_value<double>().value());  // position value
    EXPECT_EQ(0.0, state_interfaces[1]->get_value<double>().value());          // velocity
    EXPECT_EQ(
      20 * velocity_value, state_interfaces[2]->get_value<double>().value());  // position value
    EXPECT_EQ(0.0, state_interfaces[3]->get_value<double>().value());          // velocity
    EXPECT_EQ(
      20 * velocity_value, state_interfaces[4]->get_value<double>().value());  // position value
    EXPECT_EQ(0.0, state_interfaces[5]->get_value<double>().value());          // velocity
...
TEST(TestComponentInterfaces, dummy_system_default)
{
  ...
  for (auto step = 0u; step < 10; ++step)
  {
    ...
    ASSERT_TRUE(
      std::isnan(state_interfaces[si_joint1_vel]->get_value<double>().value()));  // velocity
    ASSERT_TRUE(
      std::isnan(state_interfaces[si_joint2_pos]->get_value<double>().value()));  // position value
    ASSERT_TRUE(
      std::isnan(state_interfaces[si_joint2_vel]->get_value<double>().value()));  // velocity
    ASSERT_TRUE(
      std::isnan(state_interfaces[si_joint3_pos]->get_value<double>().value()));  // position value
    ASSERT_TRUE(
      std::isnan(state_interfaces[si_joint3_vel]->get_value<double>().value()));  // velocity
  ...
  for (auto step = 0u; step < 10; ++step)
  {
    ...
    EXPECT_EQ(
      step * velocity_value,
      state_interfaces[si_joint2_pos]->get_value<double>().value());  // position value
    EXPECT_EQ(
      step ? velocity_value : 0,
      state_interfaces[si_joint2_vel]->get_value<double>().value());  // velocity
    EXPECT_EQ(
      step * velocity_value,
      state_interfaces[si_joint3_pos]->get_value<double>().value());  // position value
    EXPECT_EQ(
      step ? velocity_value : 0,
      state_interfaces[si_joint3_vel]->get_value<double>().value());  // velocity
  ...
  for (auto step = 0u; step < 10; ++step)
  {
    ...
    EXPECT_EQ(
      ...
      state_interfaces[si_joint2_pos]->get_value<double>().value());  // position value
    EXPECT_EQ(
      velocity_value, state_interfaces[si_joint2_vel]->get_value<double>().value());  // velocity
    EXPECT_EQ(
      (10 + step) * velocity_value,
      state_interfaces[si_joint3_pos]->get_value<double>().value());  // position value
    EXPECT_EQ(
      velocity_value, state_interfaces[si_joint3_vel]->get_value<double>().value());  // velocity
  ...
  for (auto step = 0u; step < 10; ++step)
  {
    ...
    EXPECT_EQ(
      20 * velocity_value,
      state_interfaces[si_joint1_pos]->get_value<double>().value());               // position value
    EXPECT_EQ(0.0, state_interfaces[si_joint1_vel]->get_value<double>().value());  // velocity
    EXPECT_EQ(
      20 * velocity_value,
      state_interfaces[si_joint2_pos]->get_value<double>().value());               // position value
    EXPECT_EQ(0.0, state_interfaces[si_joint2_vel]->get_value<double>().value());  // velocity
    EXPECT_EQ(
      20 * velocity_value,
      state_interfaces[si_joint3_pos]->get_value<double>().value());               // position value
    EXPECT_EQ(0.0, state_interfaces[si_joint3_vel]->get_value<double>().value());  // velocity
...
TEST(TestComponentInterfaces, dummy_actuator_read_error_behavior)
{
  ...
  state = actuator_hw.configure();
  EXPECT_EQ(state_interfaces[0]->get_value<double>().value(), 0.0);
  EXPECT_EQ(command_interfaces[0]->get_value<double>().value(), 0.0);
...
TEST(TestComponentInterfaces, dummy_actuator_default_read_error_behavior)
{
  ...
  auto ci_joint1_vel =
    test_components::vector_contains(command_interfaces, "joint1/velocity").second;
  state = actuator_hw.configure();
  EXPECT_EQ(state_interfaces[si_joint1_pos]->get_value<double>().value(), 0.0);
  EXPECT_EQ(command_interfaces[ci_joint1_vel]->get_value<double>().value(), 0.0);
...
TEST(TestComponentInterfaces, dummy_sensor_default_read_error_behavior)
{
  ...
  state = sensor_hw.configure();
  EXPECT_EQ(state_interfaces[si_joint1_vol]->get_value<double>().value(), 0.0);
...
TEST(TestComponentInterfaces, dummy_system_read_error_behavior)
{
  ...
  {
    EXPECT_EQ(state_interfaces[index]->get_value<double>().value(), 0.0);
  }
  for (auto index = 0ul; index < 3; ++index)
  {
    EXPECT_EQ(command_interfaces[index]->get_value<double>().value(), 0.0);
  }
  state = system_hw.activate();
  EXPECT_EQ(lifecycle_msgs::msg::State::PRIMARY_STATE_ACTIVE, state.id());
  EXPECT_EQ(hardware_interface::lifecycle_state_names::ACTIVE, state.label());
