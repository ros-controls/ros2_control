...
public:
  ...
  std::unordered_map<std::string, std::vector<std::string>> controllers_state_interfaces_map_;
...
// CM API: Called in "callback/slow"-thread
void ResourceManager::import_controller_state_interfaces(
  const std::string & controller_name, std::vector<StateInterface> & interfaces)
{
  ...
  resource_storage_->controllers_state_interfaces_map_[controller_name] = interface_names;
}

// CM API: Called in "callback/slow"-thread
std::vector<std::string> ResourceManager::get_controller_state_interface_names(
  const std::string & controller_name)
{
  return resource_storage_->controllers_state_interfaces_map_.at(controller_name);
}

// CM API: Called in "update"-thread
void ResourceManager::make_controller_state_interfaces_available(
  const std::string & controller_name)
{
  auto interface_names = resource_storage_->controllers_state_interfaces_map_.at(controller_name);
  std::lock_guard<std::recursive_mutex> guard(resource_interfaces_lock_);
...
// CM API: Called in "update"-thread
void ResourceManager::make_controller_state_interfaces_unavailable(
  const std::string & controller_name)
{
  auto interface_names = resource_storage_->controllers_state_interfaces_map_.at(controller_name);
...
// CM API: Called in "callback/slow"-thread
void ResourceManager::remove_controller_state_interfaces(const std::string & controller_name)
{
  auto interface_names = resource_storage_->controllers_state_interfaces_map_.at(controller_name);
  resource_storage_->controllers_state_interfaces_map_.erase(controller_name);
