...
namespace controller_manager {
  ...
  void ControllerManager::list_controllers_srv_cb(
    ...
    size_t ind = 0;
    std::unordered_set<std::string> controller_names;
    ...
    for (size_t i = 0; i < controllers.size(); ++i) {
      ...
      controller_names.insert(cs.name);

      controller_manager_msgs::msg::ChainedControllerState &cs_chained = response->controller_group[i];
      ...
      // add controller state to response dependning on its chained state
      if (!controllers[i].c->is_in_chained_mode()) {
        response->controller[ind++] = cs;
      } else {
        response->controller_group[ind++].controller_state = cs;
      }
    }
    for (auto cs_chained : response->controller_group){
      for (const auto& interface : cs_chained.controller_state.required_command_interfaces){
        auto name = interface.substr(0, interface.find('/'));
        cs_chained.chained_controller_names.push_back(name);
      }
    }
