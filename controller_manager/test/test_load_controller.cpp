...
TEST_F(TestLoadController, load_and_configure_one_known_controller)
{
  ...
  controller_manager::ControllerSpec abstract_test_controller = cm_->get_loaded_controllers()[0];
...
TEST_F(TestLoadController, load_and_configure_two_known_controllers)
{
  ...
  EXPECT_STREQ(controller_name1.c_str(), abstract_test_controller1.c->get_node()->get_name());
  EXPECT_EQ(
    lifecycle_msgs::msg::State::PRIMARY_STATE_UNCONFIGURED,
  ...
  EXPECT_STREQ(controller_name2.c_str(), abstract_test_controller2.c->get_node()->get_name());
  EXPECT_STREQ(controller_name2.c_str(), abstract_test_controller2.info.name.c_str());
  EXPECT_EQ(
    lifecycle_msgs::msg::State::PRIMARY_STATE_UNCONFIGURED,
...
TEST_F(TestLoadController, switch_controller_empty)
{
  ...
  {  // test switch strictness
    ...
    EXPECT_EQ(
      controller_interface::return_type::OK,
      cm_->switch_controller(
        start_controllers, stop_controllers, UNSPECIFIED, true, rclcpp::Duration(0, 0)))
