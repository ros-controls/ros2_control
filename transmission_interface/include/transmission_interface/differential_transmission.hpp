...
inline void DifferentialTransmission::actuator_to_joint()
{
  ...
  if (act_pos.size() == num_actuators() && joint_pos.size() == num_joints())
  {
    ...
    std::ignore = joint_pos[0]->set_value(
      (act_pos[0]->get_optional().value() / ar[0] + act_pos[1]->get_optional().value() / ar[1]) /
        (2.0 * jr[0]) +
      joint_offset_[0]);
  ...
  if (act_vel.size() == num_actuators() && joint_vel.size() == num_joints())
  {
    ...
    std::ignore = joint_vel[0]->set_value(
      (act_vel[0]->get_optional().value() / ar[0] + act_vel[1]->get_optional().value() / ar[1]) /
      (2.0 * jr[0]));
  ...
  if (act_eff.size() == num_actuators() && joint_eff.size() == num_joints())
  {
    ...
    std::ignore = joint_eff[0]->set_value(
      jr[0] *
      (act_eff[0]->get_optional().value() * ar[0] + act_eff[1]->get_optional().value() * ar[1]));
...
inline void DifferentialTransmission::joint_to_actuator()
{
  ...
  if (act_vel.size() == num_actuators() && joint_vel.size() == num_joints())
  {
    ...
    std::ignore = act_vel[0]->set_value(
      (joint_vel[0]->get_optional().value() * jr[0] +
       joint_vel[1]->get_optional().value() * jr[1]) *
      ar[0]);
  ...
  if (act_eff.size() == num_actuators() && joint_eff.size() == num_joints())
  {
    ...
    std::ignore = act_eff[0]->set_value(
      (joint_eff[0]->get_optional().value() / jr[0] +
       joint_eff[1]->get_optional().value() / jr[1]) /
      (2.0 * ar[0]));
