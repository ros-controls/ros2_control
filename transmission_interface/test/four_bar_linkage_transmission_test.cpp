...
TEST_F(WhiteBoxTest, MoveFirstJointOnly)
{
  ...
  // Effort interface
  {
    ...
    auto j2_handle = std::make_shared<JointHandle>(
      hardware_interface::InterfaceDescription("joint2", interface_info));
    std::ignore = a1_handle->set_value(5.0);
    std::ignore = a2_handle->set_value(10.0);

    trans.configure({j1_handle, j2_handle}, {a1_handle, a2_handle});
    trans.actuator_to_joint();
    EXPECT_THAT(100.0, DoubleNear(j1_handle->get_optional().value(), EPS));
    EXPECT_THAT(0.0, DoubleNear(j2_handle->get_optional().value(), EPS));
  }
  ...
  // Velocity interface
  {
    ...
    auto j2_handle = std::make_shared<JointHandle>(
      hardware_interface::InterfaceDescription("joint2", interface_info));
    std::ignore = a1_handle->set_value(10.0);
    std::ignore = a2_handle->set_value(5.0);
...
TEST_F(WhiteBoxTest, MoveSecondJointOnly)
{
  ...
  // Effort interface
  {
    ...
    trans.actuator_to_joint();
    EXPECT_THAT(0.0, DoubleNear(j1_handle->get_optional().value(), EPS));
    EXPECT_THAT(200.0, DoubleNear(j2_handle->get_optional().value(), EPS));
  }
...
TEST_F(WhiteBoxTest, MoveBothJoints)
{
  ...
  // Effort interface
  {
    ...
    trans.configure({j1_handle, j2_handle}, {a1_handle, a2_handle});
    trans.actuator_to_joint();
    EXPECT_THAT(-60.0, DoubleNear(j1_handle->get_optional().value(), EPS));
    EXPECT_THAT(-160.0, DoubleNear(j2_handle->get_optional().value(), EPS));
  }
  ...
  // Velocity interface
  {
    ...
    trans.configure({j1_handle, j2_handle}, {a1_handle, a2_handle});
    trans.actuator_to_joint();
    EXPECT_THAT(-0.15, DoubleNear(j1_handle->get_optional().value(), EPS));
    EXPECT_THAT(-0.025, DoubleNear(j2_handle->get_optional().value(), EPS));
  }
  ...
  // Position interface
  {
    ...
    trans.configure({j1_handle, j2_handle}, {a1_handle, a2_handle});
    trans.actuator_to_joint();
    EXPECT_THAT(-2.15, DoubleNear(j1_handle->get_optional().value(), EPS));
    EXPECT_THAT(3.975, DoubleNear(j2_handle->get_optional().value(), EPS));
  }
}
